...
def predict_new_sentence(sentence, tokenizer, model, label_to_id):
    ...
    for i, elm in enumerate(tokens):
        if elm in strings_remove:
            tokens.remove(elm)
...
def parse_ocr_text(text_to_predict=None, use_custom_text=False):
    # Load the model: It's a prerequisite that it has been trained in advanced and put in the correct folder.
    tokenizer = AutoTokenizer.from_pretrained("BERT_model")
    model = AutoModelForTokenClassification.from_pretrained("BERT_model")
    model.eval()  # Set the model to evaluation mode for predictions.
    ...
    if (use_custom_text == True) and (text_to_predict != None):
        ocr_text_objects = text_to_predict # Test on synthetic test data
    else:
        removeCoords()
        json_path = "ocr_predict.json"
        ocr_text_objects = load_json_file(json_path) # Test on ocr prediction data
    ...
    for obj in ocr_text_objects:
        ...
        # Filtering and printing the tokens corresponding to respective labels
        ...
        if (use_custom_text == True) and (text_to_predict != None):
            ...
            # strip hastags from each string
            for i, elm in enumerate(interests):
                elm = elm.replace(" ##", "") # concat sub-word tokens into words 
                elm = elm.replace('#', "") # remove left-over hastags
                interests[i] = elm
